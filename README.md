...
| Decode throughput | **29.2 tok/s C1** (MTP spec-decode, +34.5%) · 93.9 tok/s C8 (131K profile) |
| MTP acceptance | **0.50–0.55** (mean accepted length 1.50–1.55, n=1) |
| Long-ctx prefill | ~1.8–2.4K tok/s (full-1M cold prefill ≈ 9–13 min TTFT) |
| Stability | smoke 7/7 (incl. vision) · 300/300 kernel-fuzz cases (incl. spec-verify shapes) · C8 soak clean |
...
| KV pool | 213,780 tokens (1.63×) | **2,044,677 tokens (1.95×)** — 1,950,538 (1.86×) with MTP on |
| Decode C1 | **21.4 tok/s** (3 reps: 21.39/21.46/21.46) | 21.7 → **29.2 tok/s with MTP** (+34.5%) |
| Decode C2 | — | 41.0 → 39.5 tok/s with MTP (−3.6%, see note) |
| Decode C8 | **93.9 tok/s** aggregate (3 reps: 92.4/94.3/95.1) | pool-size-independent |
| Needle retrieval | — | **HIT @ 44K–1,020K** (`bench/needle.py`); 3/3 HIT @65K–512K with MTP on |
| Weights footprint | 78.3 GiB / node | same (+~0.3 GiB/node MTP drafter) |
| Config | `cluster.env.example` defaults | one commented block |

**MTP speculative decoding (1M lane, 2026-08-02 A/B, same session).**
`MTP_NUM_TOKENS=1` — the only value vLLM 0.26 accepts for this architecture
(`inkling_mtp` hard-rejects n≠1; only checkpoint depth 0 of the 8 NextN layers
loads, BF16). C1 21.72 → **29.21 tok/s (+34.5%)**; acceptance 0.503–0.550,
mean accepted length 1.50–1.55. At C2 (the lane's `MAX_NUM_SEQS`) MTP is 3.6%
*slower* — verify overhead plus the FA4 batch-split at b=2 verify steps ≈ the
acceptance gain — so the win is single-stream, which is what the 1M lane is for
(the pool fits ~1 full-length request). The KV pin drops 29.5 → 28.5 GiB to
make room for the drafter (pool 1.86×, still ample). Rollback: empty
`MTP_NUM_TOKENS` + the 29.5 GiB pin.
...
same harness class — Inkling-Small with MTP now beats that envelope on
single-stream decode (29.2 vs ~21 tok/s class at 1M context).
...
# optional MTP spec-decode (+34.5% C1): MTP_NUM_TOKENS=1 + pin 30601641984 (28.5 GiB)
```
...
pool for more system margin; 28.5 GiB when the MTP drafter is on), bring-up
traps, gates, and the fp4-KV analysis: [docs/CONTEXT-1M.md](docs/CONTEXT-1M.md).
...
bench/                  # smoke.sh, probe.sh (C1/C8), cbench.py, needle.py (long-ctx probe),
                        # acceptance.sh (spec-decode acceptance from /metrics)
